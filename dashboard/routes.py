...
def admin_revenue():
    ...
    staff_rows = analytics["staff"]
    category_rows = analytics["categories"]
    service_rows = analytics["services"]
    ...
    return render_template(
        ...
        filtered_revenue=summary["filtered_revenue"],
        average_ticket=summary["average_ticket"],
        completed_count=summary["completed_count"],
        filtered_bookings=Booking.query.filter(
            Booking.status == "completed",
            Booking.start_time >= datetime.combine(datetime.fromisoformat(analytics["range"]["start_date"]).date(), time.min),
            Booking.start_time <= datetime.combine(datetime.fromisoformat(analytics["range"]["end_date"]).date(), time.max),
        ).order_by(Booking.start_time.asc()).all(),
        period_booking_count=summary["period_booking_count"],
        filter_start_date=datetime.fromisoformat(analytics["range"]["start_date"]).date(),
        filter_end_date=datetime.fromisoformat(analytics["range"]["end_date"]).date(),
        cancellation_rate=summary["cancellation_rate"],
        no_show_rate=summary["no_show_rate"],
        completion_rate=summary["completion_rate"],
        new_customer_count=summary["new_customer_count"],
        returning_customer_count=summary["returning_customer_count"],
        repeat_customer_rate=summary["repeat_customer_rate"],
        status_counts=analytics["status_counts"],
        staff_revenue={r["name"]: r["revenue"] for r in staff_rows},
        staff_count={r["name"]: r["completed_count"] for r in staff_rows},
        category_revenue={r["name"]: r["revenue"] for r in category_rows},
        category_count={r["name"]: r["completed_count"] for r in category_rows},
        service_revenue={r["name"]: r["revenue"] for r in service_rows},
        service_count={r["name"]: r["completed_count"] for r in service_rows},
        daily_revenue=daily,
        chart_labels=list(daily.keys()),
        chart_values=[v["revenue"] for v in daily.values()],
        staff_chart_labels=[r["name"] for r in staff_rows],
        staff_chart_values=[r["revenue"] for r in staff_rows],
        category_chart_labels=[r["name"] for r in category_rows],
        category_chart_values=[r["revenue"] for r in category_rows],
        status_chart_labels=[status.replace("_", " ").title() for status in status_order],
        status_chart_values=[analytics["status_counts"].get(status, 0) for status in status_order],
        weekday_chart_labels=list(analytics["weekday_counts"].keys()),
        weekday_chart_values=list(analytics["weekday_counts"].values()),
        hour_chart_labels=list(analytics["hour_counts"].keys()),
        hour_chart_values=list(analytics["hour_counts"].values()),
    )
